...
class InterfaceController: WKInterfaceController {
    ...
    override func awake(withContext context: Any?) {
        ...
        self.updateTitle(withContext: connector.receivedApplicationContext)
    ...
    @objc func applicationContextDidChange(_ notification: Notification) {
        if let context = notification.userInfo as? [String: Any] {
    ...
    override func willActivate() {
        ...
        if let rowIndex = self.rowIndex {
            self.notes.remove(at: rowIndex)
            self.table.removeRows(at: IndexSet(integer: rowIndex))
            self.rowIndex = nil
        }
    }
